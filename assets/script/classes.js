...
const stage = {
    ...
    update(){
        ...
        const lf1 = (this.fighter1.life/this.fighter1.maxLife) *100;
        this.fighter1El.querySelector(".bar").style.width=`${lf1}%`;

        //fighter2
        this.fighter2El.querySelector(".name").innerHTML = `${this.fighter2.name} - ${this.fighter2.life.toFixed(2)} HP`
        const lf2 = (this.fighter2.life/this.fighter2.maxLife) *100;
        this.fighter2El.querySelector(".bar").style.width=`${lf2}%`;
    },
    ...
    doAttack(attacking, attacked){
        ...
        if (actualAttack > actualDefense){
            log.addMessage(`${attacking.name} deu ${actualAttack} de dano em ${attacked.name}`);
